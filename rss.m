...
%  
% Slightly altered version of rss 
%
...
if ~isequal(sizes,round(sizes)) || ~all(isfinite(sizes)) || any(sizes<0) 
    ctrlMsgUtils.error('Control:ltiobject:rss1','rss')
end

% Prob of an integrator is 0.10 for the first and 0.01 for all others
%nint = (rand(1,1)<0.10)+sum(rand(n-1,1)<0.01);

% Prob of an integrator is 0 
% OBS - different from the standard implementation of this function
nint = 0;
